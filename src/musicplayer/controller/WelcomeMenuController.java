...
public class WelcomeMenuController implements Initializable {
	...
	@FXML
	private Circle btnPlay;
	@FXML
	private Circle btnPause;
	@FXML
	...
	@FXML
	private ListView<String> lstMainTracks;
	@FXML
	private AnchorPane welcomeRootAnchor;
	@FXML
	private AnchorPane welcomeParentAnchorPane;
	@FXML
	private Label lblDisplayName;
	@FXML
	private Circle imgProfilePicture;
	@FXML
	private AnchorPane anchorNews;
	@FXML
	private Rectangle imgNoConnection;
	@FXML
	private Label lblNoConnection1;
	@FXML
	private Label lblNoConnection2;
	@FXML
	private ImageView imgSearchIcon;
	@FXML
	private ComboBox cmbSearchMusic;
	@FXML
	private ComboBox cmbSearchUser;
	@FXML
	private RadioButton rdSong;
	@FXML
	private RadioButton rdArtist;
	@FXML
	private RadioButton rdAlbum;
	@FXML
	private ImageView imgSearchUser;
	@FXML
	private Label lblNoMatchesFound;
	@FXML
	private ImageView imgRating;
	@FXML
	private ProgressIndicator progressDownload; // = new
												// ProgressBar(ProgressIndicator.INDETERMINATE_PROGRESS);
	@FXML
	private Circle btnDownload;
	@FXML
	private Circle btnLogOut;
	@FXML
	private Label lblRating;
	@FXML
	private Circle btnPen;
	@FXML
	private Circle btnAddPlaylist;
	@FXML
	private Circle btnRemovePlaylist;
	@FXML
	private ListView lstPlaylists;
	@FXML
	private ListView<MusicTrack> lstPlaylistSongs;
	@FXML
	private Circle btnMessage;
	@FXML
	private ListView<PremiumUser> lstContacts;
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		 ...
		 * toExternalForm(); welcomeRootAnchor.getStylesheets().add(css);
		 */
		...
		if (globalVariables.getPremiumUser() != null && globalVariables.getTrialuser() == null
			...
			try {
				...
				db_connector.update("premium_user", "personal_picture_path",
						"'http://www.webshare.hkr.se/FECO0002/default-user.png'",
						"user_name = '" + globalVariables.getPremiumUser().getUserName() + "'");
				globalVariables.getPremiumUser()
						.setProfilePicturePath("http://www.webshare.hkr.se/FECO0002/default-user.png");
				Image image = new Image(globalVariables.getPremiumUser().getProfilePicturePath());
				imgProfilePicture.setFill(new ImagePattern(image));
			}
			...
			try {
				...
				db_connector.update("trial_user", "personal_picture_path",
						"'http://www.webshare.hkr.se/FECO0002/default-user.png'",
						"user_name = '" + globalVariables.getTrialuser().getUserName() + "'");
				globalVariables.getTrialuser()
						.setProfilePicturePath("http://www.webshare.hkr.se/FECO0002/default-user.png");
				Image image = new Image(globalVariables.getTrialuser().getProfilePicturePath());
				imgProfilePicture.setFill(new ImagePattern(image));
			}
			...
			try {
				...
				db_connector.update("administrator", "personal_picture_path",
						"'http://www.webshare.hkr.se/FECO0002/default-user.png'",
						"staff_id = '" + globalVariables.getAdministrator().getStaffID() + "'");
				globalVariables.getAdministrator()
						.setProfilePicturePath("http://www.webshare.hkr.se/FECO0002/default-user.png");
				Image image = new Image(globalVariables.getAdministrator().getProfilePicturePath());
				imgProfilePicture.setFill(new ImagePattern(image));
			}
		...
		Tooltip.install(btnPen, new Tooltip("Add a comment to the song"));
		Image img6 = new Image("images/add.png");
		btnAddPlaylist.setFill(new ImagePattern(img6));
		Tooltip.install(btnAddPlaylist, new Tooltip("Create playlist"));
		Image img7 = new Image("images/remove.png");
		btnRemovePlaylist.setFill(new ImagePattern(img7));
		Tooltip.install(btnRemovePlaylist, new Tooltip("Remove playlist"));
		...
		lstPlaylistSongs.setCellFactory(listView -> {
			...
			deleteItem.setOnAction(event -> {
				...
				userPlaylists.get(lstPlaylists.getSelectionModel().getSelectedIndex())
						.removeMusicTracks(cell.getItem());
				db_connector.delete("playlist_has_music_track", "music_track_track_id = " + cell.getItem().getID());
		...
		lstContacts.setCellFactory(listView -> {
			...
			deleteItem.setOnAction(event -> {
				...
				boolean answer = DialogBoxManager.confirmationDialogBox("Remove contact?",
						"Are you sure you want to remove this contact?");
				if (answer) {
					globalVariables.getContactList().remove(cell.getItem());
					db_connector.delete("premium_user_has_contact",
							"contact_contact_name = '" + cell.getItem().getUserName()
									+ "' AND premium_user_user_name = '"
									+ globalVariables.getPremiumUser().getUserName() + "'");
					db_connector.delete("premium_user_has_contact",
							"premium_user_user_name = '" + cell.getItem().getUserName()
		...
		imgRating.setOnMouseEntered(event -> {
			...
			if ((globalVariables.getAdministrator() != null && globalVariables.getPremiumUser() == null
					&& globalVariables.getTrialuser() == null)
					|| (globalVariables.getPremiumUser() != null && globalVariables.getAdministrator() == null
							&& globalVariables.getTrialuser() == null)) {
				Scene scene = imgSearchIcon.getScene();
				scene.setCursor(Cursor.HAND);
				double x = event.getX();
		...
		cmbSearchMusic.getEditor().textProperty().addListener((obs, oldText, newText) -> {
			...
			if (rdSong.isSelected()) {
				if (newText.equals("")) {
					...
					for (String s : db_connector.searchMultipleResults("track_name", "music_track",
							"(track_name LIKE '%" + newText.replaceAll("'", "''") + "%' OR track_name LIKE '"
									+ newText.replaceAll("'", "''") + "%')")) {
				...
				if (newText.equals("")) {
					...
					for (String s : db_connector.searchMultipleResults("stage_name", "music_artist",
							"(stage_name LIKE '%" + newText + "%' OR stage_name LIKE '" + newText + "%')")) {
						if (!newText.equals("")) {
							sqlArrayList.add(s);
							cmbSearchMusic.getItems().clear();
							cmbSearchMusic.getItems().addAll(sqlArrayList);
				...
				if (newText.equals("")) {
					...
					for (String s : db_connector.searchMultipleResults("album_name", "album",
							"(album_name LIKE '%" + newText + "%' OR album_name LIKE '" + newText + "%')")) {
		...
		cmbSearchUser.getEditor().textProperty().addListener((obs, oldText, newText) -> {
			...
			if (rdPremium.isSelected()) {
				if (newText.equals("")) {
					...
					for (String s : db_connector.searchMultipleResults("user_name", "premium_user",
							"(user_name LIKE '%" + newText.replaceAll("'", "''") + "%' OR user_name LIKE '"
									+ newText.replaceAll("'", "''") + "%')")) {
				...
				if (newText.equals("")) {
					...
					for (String s : db_connector.searchMultipleResults("user_name", "trial_user",
							"(user_name LIKE '%" + newText + "%' OR user_name LIKE '" + newText + "%')")) {
	...
	private void clickOnPlayButton() {
		...
		if (currentStatus == MediaPlayer.Status.PAUSED || currentStatus == MediaPlayer.Status.STOPPED
				|| currentStatus == MediaPlayer.Status.READY) {
			mediaPlayer.play();
		}
	...
	private void clickOnListViewMainTracks() {
		...
		selectedItem = albumSelected.getSongs().get(lstMainTracks.getSelectionModel().getSelectedIndex())
				.getTrackName();
		...
		if (selectedItem.equals(temp)) {
			...
			mt.setID(Integer.parseInt(db_connector.search("track_id", "music_track",
					"track_name = '" + selectedItem.replaceAll("'", "''") + "'")));
			trackPlaying = mt;
	...
	private void runMediaPlayer(Path path) {
		...
		mediaPlayer.setOnReady(new Runnable() {
			...
			public void run() {
				...
				mediaPlayer.currentTimeProperty().addListener((observable, oldValue, newValue) -> {
					...
					lblTimeLeft.setText(formatTime(
							mediaPlayer.getTotalDuration().toSeconds() - mediaPlayer.getCurrentTime().toSeconds()));

					if (!tglLoop.isSelected()) {

						if (mediaPlayer.getTotalDuration().toMinutes()
								- mediaPlayer.getCurrentTime().toMinutes() <= 0.01) {
							mediaPlayer.stop();
						}
						mediaPlayer.setCycleCount(1);
		...
		try {
			InputStream input = new FileInputStream(
					new File("tmp/" + FilenameUtils.getName(url.getPath().replaceAll("%20", " "))));
	...
	private void clickOnImageView(Node n) {
		...
		int albumId = Integer
				.parseInt(db_connector.search("album_id", "album", "album_cover_path = " + "'" + imgUrl + "'"));
		...
		artistName = db_connector.search("music_artist_artist_id", "music_track",
				"track_id = " + Integer.toString(musicId));
		artistName = db_connector.search("stage_name", "music_artist", "artist_id = " + artistName);

		MusicTrack mt = new MusicTrack(db_connector.search("track_name", "music_track", "track_id = " + musicId),
				db_connector.search("track_url", "music_track", "track_id = " + musicId));
		mt.setID(musicId);
		tempArray.add(mt);
		...
		for (int i = 0; i < 15; i++) {
			...
			try {
				...
				if (musicId != 0) {
					MusicTrack mt1 = new MusicTrack(
							db_connector.search("track_name", "music_track", "track_id = " + musicId),
							db_connector.search("track_url", "music_track", "track_id = " + musicId));
					mt1.setID(musicId);
					tempArray.add(mt1);
				}
	...
	private void setImageNews() {
		...
		int counter = Integer
				.parseInt(db_connector.search("album_id", "album", "album_id = (SELECT MAX(album_id) FROM album)"));
	...
	private void clickOnSearchIcon() {
		...
		if (rdSong.isSelected()) {
			...
			if (!db_connector
					.search("track_name", "music_track", "track_name = '" + trackSearched.replaceAll("'", "''") + "'")
					.equals("")) {
				lblNoMatchesFound.setText("");
				...
				int albumId = Integer.parseInt(db_connector.search("album_album_id", "album_has_music_track",
						"music_track_track_id = " + Integer.toString(trackId)));
				String albumName = db_connector.search("album_name", "album",
				...
				musicId = Integer.parseInt(db_connector.search("music_track_track_id", "album_has_music_track",
						"album_album_id = " + Integer.toString(albumId)));
				String artistName = db_connector.search("music_artist_artist_id", "music_track",
						"track_id = " + Integer.toString(musicId));
				artistName = db_connector.search("stage_name", "music_artist", "artist_id = " + artistName);
			...
			if (!db_connector.search("album_name", "album", "album_name = '" + albumSearched + "'").equals("")) {
				lblNoMatchesFound.setText("");
			...
			if (!db_connector.search("stage_name", "music_artist", "stage_name = '" + artistSearched + "'")
					.equals("")) {
				lblNoMatchesFound.setText("");
				int artistId = Integer.parseInt(
						db_connector.search("artist_id", "music_artist", "stage_name = '" + artistSearched + "'"));
				...
				for (Integer i : musicIds) {
					if (!albumIds.contains(Integer.parseInt(db_connector.search("album_album_id",
							"album_has_music_track", "music_track_track_id = " + Integer.toString(i))))) {
						albumIds.add(Integer.parseInt(db_connector.search("album_album_id", "album_has_music_track",
								"music_track_track_id = " + Integer.toString(i))));
					}
				}
				cmbSearchMusic.show();
				cmbSearchMusic.getItems().clear();
				...
				for (Integer i : albumIds) {
					albumNames.add(db_connector.search("album_name", "album", "album_id = " + Integer.toString(i)));
				}
	...
	private void loadAlbum(String searchedItem, int songIndex) {
		...
		int albumId = Integer.parseInt(db_connector.search("album_id", "album", "album_name = '" + searchedItem + "'"));
		String albumCoverPath = db_connector.search("album_cover_path", "album",
				"album_id = " + Integer.toString(albumId));
	...
	private void writeMusicTrackToBinaryFile() {
		try (FileOutputStream fs = new FileOutputStream("MusicTrack.bin");
	...
	private void onBtnRemovePlaylistPressed(MouseEvent e) {
		...
		if (lstPlaylists.getSelectionModel().getSelectedItem() != null) {
			boolean answer = DialogBoxManager.confirmationDialogBox("Are you sure you want to remove this playlist?",
					"click ok to continue");
			if (answer) {
				db_connector.delete("playlist_has_music_track", "playlist_playlist_id = " + Integer
						.toString(userPlaylists.get(lstPlaylists.getSelectionModel().getSelectedIndex()).getId()));
				db_connector.delete("playlist", "playlist_id = " + Integer
						.toString(userPlaylists.get(lstPlaylists.getSelectionModel().getSelectedIndex()).getId()));
				setPlaylists();
			}
		} else {
	...
	private void setPlaylists() {
		...
		for (String s : db_connector.searchMultipleResults("playlist_id", "playlist", "owner = '" + userName + "'")) {
			userPlaylists.add(new Playlist(
					db_connector.search("name", "playlist", "owner = '" + userName + "' AND playlist_id = " + s),
					PrivacyLevel.values()[Integer.parseInt(db_connector.search("privacy_level_privacy_id", "playlist",
							"owner = '" + userName + "' AND playlist_id = " + s))]));
			userPlaylists.get(counter).setId(Integer.parseInt(s));
			userPlaylists.get(counter).setVisibility(
					PrivacyLevel.values()[Integer.parseInt(db_connector.search("privacy_level_privacy_id", "playlist",
							"owner = '" + userName + "' AND playlist_id = " + s))]);
			counter++;
		}
		...
		for (Playlist p : userPlaylists) {
			int nOfEntries = Integer.parseInt(db_connector.search("COUNT(playlist_playlist_id)",
					"playlist_has_music_track", "playlist_playlist_id = " + Integer.toString(p.getId())));
	...
	private void fillUpLstPlaylists(ArrayList<Playlist> userPlaylists) {
		...
		for (Playlist p : userPlaylists) {
			lstPlaylists.getItems()
					.add(p.getName() + "   - " + p.getVisibility() + "\n" + p.getNumberOfEntries() + " tracks");
		}
	}
	...
	private void readPlaylistFromBinaryFile() {
		...
		try (FileInputStream fs = new FileInputStream("PlaylistId.bin");
	...
	private void clickOnLstPlaylistSongs() {
		...
		if (lstPlaylistSongs.getItems() != null) {
			MusicTrack songInPlaylist = selectedPlaylist.getMusicTracks()
					.get(lstPlaylistSongs.getSelectionModel().getSelectedIndex());
			selectedItem = selectedPlaylist.getMusicTracks()
					.get(lstPlaylistSongs.getSelectionModel().getSelectedIndex()).getTrackName();
			// plays song only on double click
			if (selectedItem.equals(temp)) {
				lstMainTracks.getItems().clear();
				...
				currentSongRating = new Rating(songInPlaylist, Double.parseDouble(
						db_connector.search("final_rating", "rating", "rating_id = " + Integer.toString(ratingId))));
				currentSongRating.setRatingID(ratingId);
				int product = (Integer.parseInt(db_connector.search("sum_from_all_voters", "rating",
						"rating_id = " + Integer.toString(ratingId))))
				...
				temp = selectedPlaylist.getMusicTracks().get(lstPlaylistSongs.getSelectionModel().getSelectedIndex())
						.getTrackName();
			}
		}
	}
	...
	private void onImgSearchUserPressed(MouseEvent e) {
		...
		if (rdPremium.isSelected()) {
			...
			if (!db_connector
					.search("user_name", "premium_user", "user_name = '" + userSearched.replaceAll("'", "''") + "'")
					.equals("")) {
				db_connector.findPremiumUser(cmbSearchUser.getEditor().getText());
				if (GlobalVariables.getInstance().getPremiumUser() != null
						&& GlobalVariables.getInstance().getTrialuser() == null) {
					if (cmbSearchUser.getEditor().getText()
							.equals(GlobalVariables.getInstance().getPremiumUser().getUserName())) {
				...
				} else if (GlobalVariables.getInstance().getTrialuser() != null
						&& GlobalVariables.getInstance().getPremiumUser() == null) {
					if (cmbSearchUser.getEditor().getText()
							.equals(GlobalVariables.getInstance().getTrialuser().getUserName())) {
						GlobalVariables.getInstance().setOwnUserDescriptionController(new UserDescriptionController());
						GlobalVariables.getInstance().setContactDescriptionController(null);
					} else {
						GlobalVariables.getInstance().setOwnUserDescriptionController(null);
			...
			String userSearched = cmbSearchUser.getEditor().getText();
			if (!db_connector
					.search("user_name", "trial_user", "user_name = '" + userSearched.replaceAll("'", "''") + "'")
					.equals("")) {
	...
	private void setPlaylistSongs() {
		...
		if (lstPlaylists.getSelectionModel().getSelectedIndex() == tempPlaylist) {
			...
			selectedPlaylist = new Playlist(
					userPlaylists.get(lstPlaylists.getSelectionModel().getSelectedIndex()).getName(),
					userPlaylists.get(lstPlaylists.getSelectionModel().getSelectedIndex()).getVisibility());
			for (String s : songsInPlaylistId) { // 19 & 40

				songsInPlaylist.add(db_connector.search("track_name", "music_track", "track_id = " + s));
				albumId = db_connector.search("album_album_id", "album_has_music_track", "music_track_track_id = " + s);
				albumCoverPath = db_connector.search("album_cover_path", "album", "album_id = " + albumId);
				MusicTrack check = new MusicTrack(db_connector.search("track_name", "music_track", "track_id = " + s),
						db_connector.search("track_url", "music_track", "track_id = " + s));
				if (!selectedPlaylist.getMusicTracks().contains(check)) {
					selectedPlaylist.addMusicTracks(
							new MusicTrack(db_connector.search("track_name", "music_track", "track_id = " + s),
									db_connector.search("track_url", "music_track", "track_id = " + s)));
